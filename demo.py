...
y = np.log(test_df['Price'])

X1 = validation_df.drop(columns = ["Price"] , axis = 1)
y1 = np.log(validation_df['Price'])

pred = model.predict_dataframe(X , acutal_price = False)
pred1 = model.predict_dataframe(X1 , acutal_price = False)
